...
(function($) {
  ...
  var DefaultOptions = {
    ...
    NewFile: false,
    HasFunctionBar: true,
    Debug: true,    // TODO: Remember to turn it off.
    NoDefinitionsFor: [],
    ActionPreview: null,    // Fn
    ActionCreate:  null
  };
  var ActiveOptions = {};
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      ...
      if ( EditorHas.previewButton() ) {
        ...
        $('#gollum-editor #gollum-editor-preview').click(function() {
            if ( null != ActiveOptions.ActionPreview )
            {
                ActiveOptions.ActionPreview()
  ...
  var LanguageDefinition = {
    ...
    loadFor: function( markup_name, on_complete ) {
      ...
      // FIXME: Merging language JS to accomplish this ?
      // TODO:  How to load remote JS(s) in reasonable way ?
      // 
      // Means: A pagelet will has some JS(s) ready for ajax loading,
      // and server should treats these JS(s) and pagelets related files as individual "repos",
      // and set statics URLs contain these repos.
      //
      // So client can directly access them via "<site>/<pagelet-name>/<path-to-static-file>"
      // 
      // attempt to load the definition for this language
      var script_uri = Site.urlBase() + '/library/Gollum/scripts/editor/langs/' + markup_name + '.js';
      $.ajax({
                url: script_uri,
                dataType: 'script',
                cache: 'true', // to prevent timestamp added by jQuery while dataType is 'script'
